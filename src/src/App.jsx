...
const ADMIN_PASSWORD = "sicoob@potiguar";
...
function App() {
  ...
  function submitSale(){
    ...
    const p={id:editId||Date.now(),nome:form.nome.trim(),pa:form.pa.trim(),valor:Number(form.valor),data:form.data};
    setSales(prev=>editId?prev.map(s=>s.id===editId?p:s):[...prev,p]);
    setFormOpen(false);
  }
  function removeSale(id){setSales(prev=>prev.filter(s=>s.id!==id));}
  function saveGoal(){if(!goalPA.trim()||!goalVal)return;setGoals(prev=>({...prev,[goalPA.trim()]:Number(goalVal)}));setGoalPA("");setGoalVal("");}
